...
# Versiune: 5.3 - Eliminat acronim_tip_contract, corectat calcule
# =========================================================
...
def render_date_de_baza(supabase, cod_introdus, cat_sel, tip_sel, is_new, ex):
    ...
    # Returnăm doar câmpurile care există în tabela base_contracte_terti
    return {
        "cod_identificare": cod_introdus,
        "denumire_categorie": cat_sel,
        # "acronim_tip_contract" nu există în tabela TERTI, deci nu îl trimitem
...
def render_date_financiare(supabase, cod_introdus, is_new, date_existente):
    ...
    df_edit = st.data_editor(
        df, column_config=col_cfg, hide_index=True, use_container_width=True, num_rows="fixed", key="terti_fin_editor"
    )
    row = df_edit.iloc[0]
    return [{
        "cod_identificare": cod_introdus,
...
def render_echipa(supabase, cod_introdus, is_new, date_existente):
    ...
    if is_new or not date_existente:
        rows_init = [
            {"NUME ȘI PRENUME": "", "ROLUL ÎN CONTRACT": "", "PERSOANĂ DE CONTACT": False,
             "DEPARTAMENT": "", "EMAIL": "", "TELEFON MOBIL": "", "TELEFON FIX": ""}
        ...
        while len(rows_init) < NR_RANDURI_INIT:
            rows_init.append({"NUME ȘI PRENUME": "", "ROLUL ÎN CONTRACT": "", "PERSOANĂ DE CONTACT": False,
                              "DEPARTAMENT": "", "EMAIL": "", "TELEFON MOBIL": "", "TELEFON FIX": ""})
    ...
    df_edit = st.data_editor(
        df, column_config=col_cfg, hide_index=True, use_container_width=True, num_rows="fixed", key="terti_echipa_editor"
    )
    ...
    if st.button("➕ Adaugă membru", key="terti_add_membru"):
        st.session_state[key_rows].append({"NUME ȘI PRENUME": "", "ROLUL ÎN CONTRACT": "", "PERSOANĂ DE CONTACT": False,
                                           "DEPARTAMENT": "", "EMAIL": "", "TELEFON MOBIL": "", "TELEFON FIX": ""})
        st.rerun()
    rezultat = []
