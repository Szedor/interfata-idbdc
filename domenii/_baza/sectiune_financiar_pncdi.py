...
# =========================================================
# LOGICA SPECIALA:
#   - Financiarul PNCDI are mai multe randuri per proiect (cate unul per an).
...
def render(supabase, cod_introdus, is_new, date_existente):
    ...
    # Valuta si totaluri — luate din primul rand existent (sunt identice pe toate)
    ...
    df_ani_edit = st.data_editor(
        ...
        num_rows="dynamic",  # operatorul poate adauga/sterge randuri
    ...
    for _, r in df_ani_edit.iterrows():
        ...
        if not an:
            continue  # nu salvam randuri fara an
        rezultat.append({
