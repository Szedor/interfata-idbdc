...
def render(supabase, cod_introdus, is_new, date_existente):
    ...
    col_cfg = {
        ...
        "DEPARTAMENT":         st.column_config.TextColumn("🏛️ DEPARTAMENT", disabled=True),
        "EMAIL":               st.column_config.TextColumn("✉️ EMAIL", disabled=True),
        "TELEFON MOBIL":       st.column_config.TextColumn("📱 TELEFON MOBIL", disabled=True),
        "TELEFON FIX":         st.column_config.TextColumn("☎️ TELEFON FIX", disabled=True),
    }
    ...
    st.caption("ℹ️ După selectarea unui membru al echipei, așteptați afișarea automată a departamentului și a datelor de contact.")
