# =========================================================
# IDBDC/domenii/proiecte_see/financiar.py
# VERSIUNE: 1.0
# STATUS: NOU
# DATA: 2026.06.02
# =========================================================
...
#  1. VALUTA                    ← 🔖
#  2. CHELTUIELI ELIGIBILE TOTALE → costuri_totale_proiect
#  3. BUGET UPT                 → buget_upt
#  4. CONTRIBUTIE MECANISM      → contributie_finantator
...
def render(supabase, cod_introdus, is_new, date_existente):
    ...
    df = pd.DataFrame([{
        "🔖 VALUTA":                  valuta_ex,
        "CHELTUIELI ELIGIBILE TOTALE": _sf(row_ex.get("costuri_totale_proiect")),
        "BUGET UPT":                  _sf(row_ex.get("buget_upt")),
        "CONTRIBUTIE MECANISM":       _sf(row_ex.get("contributie_finantator")),
    ...
    col_cfg = {
        ...
        "CHELTUIELI ELIGIBILE TOTALE": st.column_config.NumberColumn(
            "CHELTUIELI ELIGIBILE TOTALE", format="%,.2f", min_value=0.0
        ),
        "BUGET UPT": st.column_config.NumberColumn(
            "BUGET UPT", format="%,.2f", min_value=0.0
        ),
        "CONTRIBUTIE MECANISM": st.column_config.NumberColumn(
            "CONTRIBUTIE MECANISM", format="%,.2f", min_value=0.0
        ),
    ...
    return [{
        ...
        "valuta":                 row["🔖 VALUTA"],
        "costuri_totale_proiect": float(row["CHELTUIELI ELIGIBILE TOTALE"] or 0),
        "buget_upt":              float(row["BUGET UPT"] or 0),
        "contributie_finantator": float(row["CONTRIBUTIE MECANISM"] or 0),
