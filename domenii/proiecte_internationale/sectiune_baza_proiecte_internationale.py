...
# VERSIUNE: 1.0
# STATUS: NOU - Date de bază pentru Proiecte INTERNATIONALE
# DATA: 2026.05.09
# =========================================================
...
def render(supabase, cod_introdus, cat_sel, tip_label, tabela_nume, is_new, date_existente):
    ...
    row_init = {
        ...
        "SCOR EVALUARE":           date_existente.get("scor_evaluare", ""),
        ...
        "SCHEMA DE FINANTARE":     date_existente.get("schema_de_finantare", ""),
        "APELUL":                  date_existente.get("identificare_apel", ""),
        "DATA LIMITA DEPUNERE":    to_date(date_existente.get("data_inchidere_apel")),
        "PROGRAM DE FINANTARE":    date_existente.get("program_finantare", ""),
        "TEMA / TOPIC":            date_existente.get("tema_topic", ""),
    ...
    return {
        ...
        "scor_evaluare":           str(row["SCOR EVALUARE"]).strip()           if row["SCOR EVALUARE"]           else None,
        "numar_participanti":      str(row["NR.PARTICIPANTI"]).strip()         if row["NR.PARTICIPANTI"]         else None,
        "denumire_participanti":   str(row["DENUMIRE PARTICIPANTI"]).strip()   if row["DENUMIRE PARTICIPANTI"]   else None,
        "rol_upt":                 str(row["ROL UPT"]).strip()                 if row["ROL UPT"]                 else None,
        "schema_de_finantare":     str(row["SCHEMA DE FINANTARE"]).strip()     if row["SCHEMA DE FINANTARE"]     else None,
        "identificare_apel":       str(row["APELUL"]).strip()                  if row["APELUL"]                  else None,
        "data_inchidere_apel":     _fmt_date(row["DATA LIMITA DEPUNERE"]),
        "program_finantare":       str(row["PROGRAM DE FINANTARE"]).strip()    if row["PROGRAM DE FINANTARE"]    else None,
        "tema_topic":              str(row["TEMA / TOPIC"]).strip()            if row["TEMA / TOPIC"]            else None,
