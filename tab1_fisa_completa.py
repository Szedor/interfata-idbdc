def _render_echipa_compact(rows: list, cod_ctx: str = "", supabase=None):
    ...
    if persoane_cont:
        ...
        for r in persoane_cont:
            ...
            # 🔧 MODIFICAT: același font pentru toate informațiile
            contact_html = ""
            if contact:
                contact_html = "  ·  ".join(_html.escape(c) for c in contact)
            ...
            st.markdown(
                ...
                f"<span style='font-weight:800;color:#ffffff;font-size:0.95rem;'>⭐ {_html.escape(txt)}</span><br>"
                f"<span style='font-weight:400;color:#ffffff;font-size:0.95rem;'>{contact_html}</span></div>",
                unsafe_allow_html=True,
            )
