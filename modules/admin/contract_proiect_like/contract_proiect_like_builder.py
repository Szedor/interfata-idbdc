...
class ContractProiectLikeBuilder:
    ...
    def build(
        ...
        *,
        ...
        loaded, exists_map = self._load_all_tables(cod=ctx.cod, tabele=tabele)
        ...
        *,
        ...
        for _, table_name in tabele:
            ...
            if table_name == "com_date_financiare" and config.get("uses_fdi_financial", False):
                df_for_save = st.session_state.get(state_key_raw(tabela_baza), pd.DataFrame()).copy()
                ...
                if isinstance(df_raw_original, pd.DataFrame) and not df_raw_original.empty:
                    try:
                        df_for_save = merge_back_control_cols(df_edit_visible.copy(), df_raw_original.copy())
                    except Exception:
                        df_for_save = df_edit_visible.copy()
                else:
                    df_for_save = df_edit_visible.copy()
    ...
    def _resolve_tabs(self, config: dict[str, Any]) -> list[tuple[str, str]]:
        ...
        if not tabs:
            raise ValueError("Config invalid: lipsesc tab-urile")
        return tabs
        ...
        if not data:
            try:
                if "id_proiect_contract_sursa" in cols:
                    res = self.supabase.table(table_name).select("*").eq("id_proiect_contract_sursa", cod).execute()
                    data = res.data or []
            except Exception:
                data = []
    ...
    def _assert_action_is_valid(
        ...
        *,
        ...
        if actiune == "Modificare / completare fișă existentă" and not base_exists:
            raise ValueError("Nu există fișă pentru acest cod. Alege «Fișă nouă».")
        if actiune == "Fișă nouă" and base_exists:
            raise ValueError("Există deja fișă pentru acest cod. Alege «Modificare».")
        ...
        *,
        ...
        for _, table_name in tabele:
            ...
            if table_name == "com_date_financiare":
                df_full = self._prepare_financial_df(cod=cod, tabela_baza=tabela_baza, loaded=loaded, config=config)
            elif table_name == "com_aspecte_tehnice":
                df_full = self._prepare_tehnic_df(cod=cod, loaded=loaded, config=config)
            else:
                ...
                if table_name == tabela_baza:
                    df_full = self._autofill_base_table_fields(df=df_full, cat_admin=cat_admin, tip_admin=tip_admin)
                else:
                    df_full = self._assign_link_fields(df_full, cod)
            df_for_editor = prepare_contract_proiect_like_df_for_editor(table_name=table_name, df=df_full)
            ...
            prepared[table_name] = {"df": df_for_editor, "column_config": column_config}
        return prepared
    ...
    def _prepare_tehnic_df(
        ...
        *,
        ...
        df_tehnic, cols_tehnic = loaded.get("com_aspecte_tehnice", (pd.DataFrame(), []))
    ...
    def _assign_link_fields(self, df: pd.DataFrame, cod: str) -> pd.DataFrame:
        out = df.copy()
        if "cod_identificare" in out.columns:
            out["cod_identificare"] = out["cod_identificare"].fillna(cod).astype(str)
