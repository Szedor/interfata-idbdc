# modules/admin/orchestrator/motor_admin.py
...
def porneste_motorul(supabase):
    ...
    def direct_save_all_tables(items: list, operator: str) -> tuple[bool, str]:
        ...
        for table_name, payloads in by_table.items():
            try:
                ...
                # upsert
                try:
                    check = supabase.table(table_name).select("cod_identificare").eq("cod_identificare", cod0).limit(1).execute()
                    exists = bool(check.data)
                except Exception:
                    exists = False

                if exists:
                    supabase.table(table_name).update(clean_payloads[0]).eq("cod_identificare", cod0).execute()
                else:
                    supabase.table(table_name).insert(clean_payloads[0]).execute()
                ok_any = True
        ...
        for t in table_names:
            try:
                ...
                if "observatii_idbdc" in cols:
                    try:
                        cur = supabase.table(t).select("observatii_idbdc").eq("cod_identificare", cod).limit(1).execute()
                        existing = ""
    ...
    st.markdown(
        """
        <style>
          [data-testid="stSidebar"] { background: #0b2a52 !important; border-right: 2px solid rgba(255,255,255,0.20); }
    ...
    if not tip_admin:
        st.info("Selectează tipul de contract/proiect.")
        return
    ...
    if tip_admin not in entity_type_map:
        st.info(f"Tipul {tip_admin} nu este suportat în această versiune.")
        return
    ...
    try:
        result = build_contract_proiect_like(
            ...
            cod=cod,
            actiune=actiune,
            entity_type=entity_type_map[tip_admin],
            cat_admin=cat_admin,
            tip_admin=tip_admin,
        )
    except Exception as e:
    ...
    if prev_cod != cod or prev_tabela != result["tabela_baza"]:
        for _, tn in tabele:
            for k in (f"df_admin__{tn}", f"df_admin_raw__{tn}", f"toggle_deblocat_{prev_cod}", f"echipa_reunited_{prev_cod}"):
                if k in st.session_state:
                    del st.session_state[k]
        st.session_state[_prev_cod_key] = cod
        st.session_state[_prev_tabela_key] = result["tabela_baza"]
                ...
                if ok and "parțial" in msg.lower():
                    ...
                    for _, table_name in tabele:
                        for k in (state_key(table_name), state_key_raw(table_name)):
                            if k in st.session_state:
                                del st.session_state[k]
                    if f"echipa_reunited_{cod}" in st.session_state:
                        del st.session_state[f"echipa_reunited_{cod}"]
                    st.session_state["admin_msg"] = ("success", msg)
                else:
