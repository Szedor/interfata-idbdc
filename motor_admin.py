...
def porneste_motorul(supabase):
    ...
    def fetch_map(table_name: str, key_col: str, val_col: str) -> dict:
        """Returneaza mapare: key_col -> val_col"""
        try:
    ...
    map_pi_to_perioada_ani = fetch_map(
        "nom_prop_intelect",
    ...
    # Session-state DF (ca sa putem completa automat si sa se vada imediat)
    ...
    # ----------------------------
    # Auto-completare REACTIVA (se vede imediat)
    # ----------------------------
    changed = False
    df_auto = ed_df.copy()

    # Evenimente: natura -> clasificare (doar daca tinta e goala)
    if nume_tabela == "base_evenimente_stiintifice":
        if "natura_eveniment" in df_auto.columns and "clasificare_eveniment" in df_auto.columns:
            for i in range(len(df_auto)):
                ...
                valoare = map_natura_to_clasificare.get(nat_key)
                if valoare is None:
                    continue
                tinta = df_auto.at[i, "clasificare_eveniment"]
                if pd.isna(tinta) or str(tinta).strip() == "":
                    df_auto.at[i, "clasificare_eveniment"] = valoare
                    changed = True

    # PI: acronim -> perioada_valabilitate (din nom: perioada_valabilitate_ani)
    if nume_tabela == "base_prop_intelect":
        if "acronim_prop_intelect" in df_auto.columns and "perioada_valabilitate" in df_auto.columns:
            for i in range(len(df_auto)):
                ...
                if pd.isna(ac) or str(ac).strip() == "":
                    continue
                ac_key = str(ac).strip()
                valoare = map_pi_to_perioada_ani.get(ac_key)
                if valoare is None:
                    continue
                tinta = df_auto.at[i, "perioada_valabilitate"]
                if pd.isna(tinta) or str(tinta).strip() == "":
                    df_auto.at[i, "perioada_valabilitate"] = valoare
                    changed = True
