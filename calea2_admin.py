...
def run():
    ...
    with c2:
        # [1] Caseta tip proiecte cu cele 8 opțiuni derulabile
        optiuni_tip = ["", "FDI", "PNCDI", "ORIZONT", "POC", "POCU", "POIM", "POCA", "PNDR"]
        tip_admin = st.selectbox("Tip:", optiuni_tip, key="admin_tip")
    with c3:
    ...
    if cat_admin != "":
        ...
        # [2] BUTOANE CRUD TOATE DEASUPRA TABELULUI
        ...
        with col_v:
            if st.button("VALIDARE"):
                st.info("Validare în curs...")
                
        with col_d:
            if st.button("ȘTERGERE"):
                st.error("Ștergere solicitată...")

        with col_a:
            if st.button("ANULARE"):
                if f'df_{nume_tabela}' in st.session_state:
