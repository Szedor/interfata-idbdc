...
def run():
    ...
    if cat_admin != "":
        ...
        # Butoane CRUD
        col_n, col_sp, col_s, col_v, col_d = st.columns([1, 6, 1, 1, 1])
        ...
        with col_n:
            if st.button("➕", help="Adaugă rând nou sus"):
                if not df_curent.empty:
                    ...
                    st.warning("Încarcă datele pentru a adăuga rânduri.")
                st.rerun()
        ...
        # Afișare butoane acțiune la schimbare
        if not st.session_state[f'df_{nume_tabela}'].equals(edited_df):
            with col_s:
                if st.button("💾"): st.success("Gata de salvare")
            with col_v:
                if st.button("🛡️"): st.info("Validat")
            with col_d:
                if st.button("🗑️"): st.error("Marcat pentru ștergere")
